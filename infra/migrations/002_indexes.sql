...
CREATE INDEX IF NOT EXISTS idx_trades_active ON trades(state, opened_at DESC) WHERE state = 'OPEN';
...
CREATE INDEX IF NOT EXISTS idx_state_machines_state ON state_machine_snapshots(state);

-- Query by creation time for cleanup/expiration
CREATE INDEX IF NOT EXISTS idx_state_machines_created_at ON state_machine_snapshots(created_at DESC);

-- ============================================================================
-- DAILY STATE INDEXES
-- ============================================================================
-- Query by date range for analytics (already has primary key, but explicit for clarity)
CREATE INDEX IF NOT EXISTS idx_daily_state_date ON daily_state(date DESC);

-- ============================================================================
-- HTF BIAS HISTORY INDEXES
-- ============================================================================
-- Query recent bias for context
CREATE INDEX IF NOT EXISTS idx_htf_bias_timestamp ON htf_bias_history(timestamp DESC);

-- Query by bias type for analytics
CREATE INDEX IF NOT EXISTS idx_htf_bias_bias ON htf_bias_history(bias);
