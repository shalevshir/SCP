...
def detect_structure_conflict(
    ...
    """Detect if 1H and 15M structures are in conflict.

    A conflict occurs when the two timeframes show opposite directional bias,
    indicating lack of alignment across timeframes.
    ...
    Returns:
        ...
        - is_conflict: True if structures conflict
        - reason: Description of conflict, or None if no conflict

    Logic:
        - Bullish 1H (HH, HL) + Bearish 15M (LH, LL) = conflict
        - Bearish 1H (LH, LL) + Bullish 15M (HH, HL) = conflict
        - Same direction or neutral = no conflict

    Example:
        >>> detect_structure_conflict("HH", "LH")
        (True, "1H bullish (HH) conflicts with 15M bearish (LH)")
    """
    # Handle None or empty values
    if not structure_1h or not structure_15m:
        return False, None

    # Define bullish and bearish structure labels
    bullish_structures = {"HH", "HL"}
    bearish_structures = {"LH", "LL"}

    # Determine 1H direction
    is_1h_bullish = structure_1h in bullish_structures
    is_1h_bearish = structure_1h in bearish_structures

    # Determine 15M direction
    is_15m_bullish = structure_15m in bullish_structures
    is_15m_bearish = structure_15m in bearish_structures
