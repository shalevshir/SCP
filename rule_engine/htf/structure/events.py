...
def extract_structure_candles(
    ...
    if df_1h is not None and choch_series is not None:
        result["choch_candle"] = extract_choch_candle(df_1h, choch_series, current_timestamp)
    
    # Extract sweep candle
    if df_15m is not None and sweep_events is not None:
        result["sweep_candle"] = extract_sweep_candle(df_15m, sweep_events, current_timestamp)
