...
class TestWarmupPeriod:
    ...
    async def test_warmup_blocks_signals_during_first_n_bars(self) -> None:
        ...
        signal_engine.generate.return_value = SignalResult(
            signal_msg=a_plus_signal_msg, raw_signal=raw_signal, rejection_reason=None
        )
    ...
    async def test_warmup_allows_signals_after_warmup_complete(self) -> None:
        ...
        assert (
            signal_publisher.publish.call_count == 10
    ...
    @pytest.mark.skip(
        reason="Env var override requires specific pydantic settings prefix - not critical for warmup functionality"
    )
