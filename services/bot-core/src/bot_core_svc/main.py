...
async def process_feature_message(
    ...
    if result.signal_msg is not None:
        ...
        core_metrics.current_setup_type.labels(mode=mode, service=service).set(
            setup_type_value
        )
        ...
        if result.rejection_reason:
            core_metrics.record_signal_rejection(result.rejection_reason, mode, service)
...
async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
    ...
    processing_task = asyncio.create_task(process_features(redis_client, db_pool))
