...
async def test_ib_data_client_on_tick(mock_ib_insync):
    ...
    ticker = MockTicker(last=2650.5, last_size=10.0, time=expected_time)
...
async def test_ib_data_client_on_tick_fallback_timestamp(mock_ib_insync):
    ...
    ticker = MockTicker(
        last=2650.5, last_size=10.0, time=None, last_timestamp=expected_timestamp
    )
...
async def test_ib_data_client_stream_ticks(mock_ib_insync):
    ...
    stream_task = asyncio.create_task(anext(client.stream_ticks().__aiter__()))
...
async def test_resilient_client_no_stale_ticks_after_reconnection():
    ...
    assert datetime(2025, 1, 15, 10, 2, 0, tzinfo=UTC) not in [
        t.timestamp for t in ticks
    ]
