...
class TestSessionEventPublisher:
    ...
    async def test_session_date_uses_timezone_not_utc(self) -> None:
        ...
        candle_utc = CandleMessage(
            timestamp=datetime(2025, 1, 14, 1, 0, tzinfo=UTC),  # 1 AM UTC = 8 PM ET Jan 13
            symbol="GC",
            timeframe="1m",
            open=2650.0,
            high=2652.0,
    ...
    async def test_session_date_handles_daylight_saving_time(self) -> None:
        ...
        candle_dst = CandleMessage(
            timestamp=datetime(2025, 7, 15, 1, 0, tzinfo=UTC),  # 1 AM UTC = 9 PM ET Jul 14
            symbol="GC",
            timeframe="1m",
            open=2650.0,
            high=2652.0,
    ...
    async def test_session_closed_event_uses_timezone(self) -> None:
        ...
        candle_close = CandleMessage(
            timestamp=datetime(2025, 1, 17, 22, 30, tzinfo=UTC),  # 5:30 PM ET Jan 17 (standard time)
            symbol="GC",
            timeframe="1m",
            open=2650.0,
            high=2652.0,
