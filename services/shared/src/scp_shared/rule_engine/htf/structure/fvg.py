...
def detect_fvg(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fvg_df = (
        pd.DataFrame(fvgs, columns=result_columns)
        if fvgs
...
def check_fvg_filled(df: pd.DataFrame, fvg_df: pd.DataFrame) -> pd.DataFrame:
...
class FVGStateTracker:
    ...
    def update(
        self, df: pd.DataFrame, current_timestamp: pd.Timestamp | datetime
    ) -> None:
                    ...
                    if not self.fvgs[fvg_key].filled:
                        ...
                        if pd.notna(row.get("fill_index")):
                            ...
                            if fill_idx < len(df_filtered):
                                self.fvgs[fvg_key].fill_timestamp = df_filtered.index[
                                    fill_idx
                                ]
    ...
    def get_active_fvgs(
        self, current_timestamp: pd.Timestamp | datetime
    ) -> list[FVGState]:
