...
from scp_shared.rule_engine.htf.features import (
    ...
    if approach == "streaming":
        ...
        def htf_bias_func(features_1m: pd.Series, context: dict) -> HTFBias:
            ...
            if not sync_bar:
                ...
                return HTFBias(
                    bias="neutral", direction="neutral", score=0.0, confidence="low"
                )
            ...
            if sync_bar.htf_15m:
                ...
                if (
                    not candle_buffer_15m
                    or candle_buffer_15m[-1].timestamp != current_15m_candle.timestamp
                ):
                ...
                if (
                    ...
                    try:
                        from scp_shared.rule_engine.htf.structure import (
                            detect_liquidity_sweeps,
                            detect_swings,
                        )

                        swing_highs_15m, swing_lows_15m = detect_swings(
                            df_15m, lookback=swing_window
                        )
                        ...
                        logger.debug(
                            f"Failed to detect liquidity sweeps in streaming mode: {e}"
                        )
                        sweep_events_15m = None
        ...
        def htf_bias_func(features_1m: pd.Series, context: dict) -> HTFBias:
            ...
            if sync_bar.htf_15m and features_15m_df is not None:
                ...
                if htf_15m_ts in features_15m_df.index:
                    ...
                    valid_timestamps = features_15m_df.index[
                        features_15m_df.index <= htf_15m_ts
                    ]
            ...
            if sync_bar.htf_1h and features_1h_df is not None:
                ...
                if htf_1h_ts in features_1h_df.index:
                    ...
                    # Find closest timestamp (forward-fill behavior)
                    valid_timestamps = features_1h_df.index[
                        features_1h_df.index <= htf_1h_ts
                    ]
                    if len(valid_timestamps) > 0:
                        closest_ts = valid_timestamps.max()
            ...
            if sweep_events_15m_series is not None and sync_bar.htf_15m:
                ...
                if htf_15m_ts in sweep_events_15m_series.index:
                    sweep_events_for_timestamp = sweep_events_15m_series.loc[
                        :htf_15m_ts
                    ]
                else:
                    # Find closest timestamp (forward-fill behavior)
