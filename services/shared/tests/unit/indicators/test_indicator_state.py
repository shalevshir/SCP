...
from scp_shared.indicators.state import (
    DXYCorrelationState,
    EMAState,
    FeatureState,
    RSIState,
...
class TestVWAPState:
    ...
    def test_multiple_candles(self):
        ...
        candles = [
            Candle(
                datetime(2025, 1, 1, 10, i, tzinfo=UTC),
                100.0,
                102.0,
                98.0,
                101.0,
    ...
    def test_session_reset(self):
        ...
        candle1 = Candle(
            datetime(2025, 1, 15, 13, 0, tzinfo=UTC),  # 08:00 ET
            100.0,
            102.0,
            98.0,
            101.0,
        ...
        state.update(candle1)

        # Still before 08:20 ET (same session)
        candle2 = Candle(
            datetime(2025, 1, 15, 13, 10, tzinfo=UTC),  # 08:10 ET
            101.0,
            103.0,
            99.0,
            102.0,
        ...
        state.update(candle2)

        # At 08:20 ET (RESET - new session starts)
        candle3 = Candle(
            datetime(2025, 1, 15, 13, 20, tzinfo=UTC),  # 08:20 ET
            100.0,
            102.0,
            98.0,
            101.0,
...
class TestDXYCorrelationState:
    ...
    def test_warmup_period(self):
        ...
        for i in range(49):
            corr = state.update(100.0 + i, 50.0 - i * 0.5, datetime.now(UTC))
            assert corr is None
    ...
    def test_no_correlation(self):
        ...
        for i in range(50):
            ...
            corr = state.update(
                gc_price, dxy_price, datetime.now(UTC) + timedelta(minutes=i)
            )
    ...
    def test_missing_prices(self):
        ...
        corr1 = state.update(None, 50.0, datetime.now(UTC))
        assert corr1 is None

        corr2 = state.update(100.0, None, datetime.now(UTC))
...
class TestFeatureState:
    ...
    def test_out_of_order_warning(self, caplog):
        ...
        # First candle
        gc_candle1 = Candle(
            datetime(2025, 1, 1, 10, 1, tzinfo=UTC),
            100.0,
            102.0,
            98.0,
            101.0,
