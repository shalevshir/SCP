...
import pandas as pd
import pytest
from scp_shared.rule_engine.htf.vwap.trend import validate_vwap_trend
...
class TestVWAPTrendValidation:
    ...
    def test_just_under_threshold_no_confirmation(self):
        ...
        df = pd.DataFrame(
            {"close": [2645, 2650], "vwap": [2640, 2642]}  # Only 2 candles
        )
    ...
    def test_dataframe_shorter_than_min_candles(self):
        ...
        df = pd.DataFrame({"close": [2645], "vwap": [2640]})
    ...
    def test_min_candles_one_immediate_confirmation(self):
        ...
        df = pd.DataFrame(
            {"close": [2645, 2650, 2640, 2655], "vwap": [2640, 2642, 2642, 2642]}
        )
    ...
    def test_invalid_min_candles_zero(self):
        ...
        df = pd.DataFrame({"close": [2645, 2650], "vwap": [2640, 2642]})
    ...
    def test_missing_close_column(self):
        ...
        df = pd.DataFrame({"vwap": [2640, 2642, 2645]})
    ...
    def test_missing_vwap_column(self):
        ...
        df = pd.DataFrame({"close": [2645, 2650, 2655]})
    ...
    def test_all_nan_returns_all_false(self):
        ...
        df = pd.DataFrame(
            {"close": [np.nan, np.nan, np.nan], "vwap": [np.nan, np.nan, np.nan]}
        )
    ...
    def test_configurable_min_candles_three(self):
        ...
        df = pd.DataFrame(
            {"close": [2645, 2650, 2655, 2660], "vwap": [2640, 2642, 2645, 2648]}
        )
    ...
    def test_equal_to_vwap_not_confirmed(self):
        ...
        df = pd.DataFrame(
            {"close": [2645, 2642, 2655, 2660], "vwap": [2640, 2642, 2645, 2648]}
        )
