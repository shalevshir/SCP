...
class TestHTFCandleAggregator:
    ...
    def test_15m_boundary_detection(self):
        ...
        # Not a boundary
        assert not agg.is_15m_boundary(
            datetime(2025, 1, 15, 10, 5, tzinfo=timezone.utc)
        )
        assert not agg.is_15m_boundary(
            datetime(2025, 1, 15, 10, 0, tzinfo=timezone.utc)
        )
    ...
    def test_15m_aggregation_basic(self):
        ...
        for i in range(15):
            ...
            if i < 14:
                ...
                assert candle_15m.timestamp == datetime(
    ...
    def test_1h_aggregation_basic(self):
        ...
        for i in range(60):
            ...
            if i == 59:
                ...
                assert candle_15m.timeframe == "15m"
                assert candle_15m.timestamp == datetime(
                    2025, 1, 15, 10, 45, tzinfo=timezone.utc
                )

                assert candle_1h.timeframe == "1h"
                assert candle_1h.timestamp == datetime(
                    2025, 1, 15, 10, 0, tzinfo=timezone.utc
                )
    ...
    def test_multiple_15m_periods(self):
        ...
        assert candles_15m[0].timestamp == datetime(
            2025, 1, 15, 10, 0, tzinfo=timezone.utc
        )
        assert candles_15m[1].timestamp == datetime(
            2025, 1, 15, 10, 15, tzinfo=timezone.utc
        )
    ...
    def test_15m_and_1h_emitted_together(self):
        ...
        assert candles_15m[2].timestamp == datetime(
            2025, 1, 15, 10, 30, tzinfo=timezone.utc
        )
        assert candles_15m[3].timestamp == datetime(
            2025, 1, 15, 10, 45, tzinfo=timezone.utc
        )

        # Should have 1 x 1h candle (at 59)
        assert len(candles_1h) == 1
        assert candles_1h[0].timestamp == datetime(
            2025, 1, 15, 10, 0, tzinfo=timezone.utc
        )
    ...
    def test_missed_boundary_candle_15m(self):
        ...
        assert agg.current_15m_start == datetime(
            2025, 1, 15, 10, 0, tzinfo=timezone.utc
        )
