...
class TestFeatureRepositorySaveFeatures:
    ...
    @pytest.mark.asyncio
    async def test_save_features_executes_upsert(
        self, mock_db_pool: MagicMock
    ) -> None:
...
class TestFeatureRepositoryLoadRecentCandles:
    ...
    async def test_load_recent_candles_pairs_by_timestamp(
        ...
        mock_db_pool.fetch = AsyncMock(side_effect=[
            # GC candles
            [
                {
                    "timestamp": ts,
    ...
    async def test_load_recent_candles_skips_unmatched_timestamps(
        ...
        mock_db_pool.fetch = AsyncMock(side_effect=[
            ...
            [
                {"timestamp": ts1, "open": 2650.0, "high": 2655.0, "low": 2648.0, "close": 2654.0, "volume": 1000.0},
                {"timestamp": ts2, "open": 2654.0, "high": 2658.0, "low": 2652.0, "close": 2656.0, "volume": 1100.0},
            ],
            # DXY candles - has ts1 and ts3 (no ts2)
            [
                {"timestamp": ts1, "open": 103.5, "high": 103.7, "low": 103.4, "close": 103.6, "volume": 500.0},
                {"timestamp": ts3, "open": 103.6, "high": 103.8, "low": 103.5, "close": 103.7, "volume": 600.0},
            ],
        ])
    ...
    async def test_load_recent_candles_converts_types(
        ...
        mock_db_pool.fetch = AsyncMock(side_effect=[
            [{"timestamp": ts, "open": "2650.5", "high": "2655.0", "low": "2648.0", "close": "2654.0", "volume": "1000.0"}],
            [{"timestamp": ts, "open": "103.5", "high": "103.7", "low": "103.4", "close": "103.6", "volume": "500.0"}],
        ])
