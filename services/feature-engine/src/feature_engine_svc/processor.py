...
class FeatureProcessor:
    ...
    def process(
        ...
        # FIX: Read from dxy_corr_micro (5-bar window) - the key is dxy_corr_micro, not dxy_5m_corr
        dxy_5m_corr = self._safe_float(features_series.get("dxy_corr_micro"))
        ...
        return FeaturesMessage(
            ...
            vwap_deviation_normalized=self._safe_float(
                features_series.get("vwap_deviation_normalized")
            ),
            # Trend indicators
            ...
            dxy_structure=features_series.get("dxy_structure_label")
            or features_series.get("dxy_structure"),
            # Structure labels
            ...
            structure_clarity=self._safe_float(
                features_series.get("structure_clarity")
            ),
            ...
            reclaim_candle_low=self._safe_float(
                features_series.get("reclaim_candle_low")
            ),
            reclaim_candle_high=self._safe_float(
                features_series.get("reclaim_candle_high")
            ),
            reclaim_candle_idx=self._safe_int(
                features_series.get("reclaim_candle_idx")
            ),
            ...
            immediate_resistance=self._safe_float(
                features_series.get("immediate_resistance")
            ),
            immediate_support=self._safe_float(
                features_series.get("immediate_support")
            ),
            prior_session_high=self._safe_float(
                features_series.get("prior_session_high")
            ),
            prior_session_low=self._safe_float(
                features_series.get("prior_session_low")
            ),
            nearest_liquidity_long=self._safe_float(
                features_series.get("nearest_swing_high_above")
            ),
            nearest_liquidity_short=self._safe_float(
                features_series.get("nearest_swing_low_below")
            ),
            ...
            second_confirmation_long=bool(
                features_series.get("second_confirmation_long", False)
            ),
            second_confirmation_short=bool(
                features_series.get("second_confirmation_short", False)
            ),
            # VWAP acceptance tracking (SOP alignment)
            bars_near_vwap=self._safe_int(features_series.get("bars_near_vwap")),
