...
async def test_kill_switch_blocks_pending_signals_execution():
    ...
    assert (
        signals_executed is False
    ), "Pending signals should not execute when kill switch is active"
...
async def test_kill_switch_clears_pending_signals():
    ...
    assert (
        len(pending_signals_after_kill) == 0
    ), "Pending signals should be cleared on kill"
    assert (
        len(pending_signals_after_resume) == 0
    ), "Pending signals should be cleared on resume"
