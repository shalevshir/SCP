...
class MultiTimeframeData:
    ...
    def get_bar(self, timestamp: datetime) -> SynchronizedBar | None:
...
class MultiTimeframeSyncLayer:
    ...
    def _synchronize_all_feeds(
        ...
        if len(common_timestamps) == 0:
            logger.warning(
                "No overlapping timestamps between GC and DXY execution data"
            )
            return []
        ...
        for ts in common_timestamps:
            ...
            exec_gc_bar = self._df_row_to_candle(
                exec_gc_row, "GC", self.execution_timeframe
            )
            exec_dxy_bar = self._df_row_to_candle(
                exec_dxy_row, "DXY", self.execution_timeframe
            )
            ...
            if "15m" in self.htf_timeframes:
                ...
                if (
                    htf_15m_gc_series is not None
                    and htf_15m_dxy_series is not None
                    and isinstance(htf_15m_gc_series, pd.Series)
                    and isinstance(htf_15m_dxy_series, pd.Series)
                ):
                    htf_15m_gc_bar = self._series_to_candle(
                        htf_15m_gc_series, "GC", "15m"
                    )
                    htf_15m_dxy_bar = self._series_to_candle(
                        htf_15m_dxy_series, "DXY", "15m"
                    )
            ...
            if "1h" in self.htf_timeframes:
                ...
                if (
                    htf_1h_gc_series is not None
                    and htf_1h_dxy_series is not None
                    and isinstance(htf_1h_gc_series, pd.Series)
                    and isinstance(htf_1h_dxy_series, pd.Series)
                ):
                    htf_1h_gc_bar = self._series_to_candle(htf_1h_gc_series, "GC", "1h")
                    htf_1h_dxy_bar = self._series_to_candle(
                        htf_1h_dxy_series, "DXY", "1h"
                    )
    ...
    def _align_htf_to_execution(
        ...
        if htf_df.empty:
            logger.debug(
                f"No HTF data for {timeframe}, returning None for all timestamps"
            )
