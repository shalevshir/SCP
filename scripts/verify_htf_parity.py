...
def compare_htf_bias(micro_bias, legacy_bias) -> dict:
    ...
    if micro_bias.bias != legacy_bias.bias:
        differences.append(f"bias: micro={micro_bias.bias}, legacy={legacy_bias.bias}")
...
def main():
    ...
    for idx, (gc_row, dxy_row) in enumerate(
        zip(gc_df.itertuples(), dxy_df.itertuples())
    ):
                ...
                "micro_bias": (
                    {
                        ...
                        "structure_15m": (
                            micro_bias.structure_15m if micro_bias else None
                        ),
                    }
                    if micro_bias
                    else None
                ...
                "legacy_bias": (
                    {
                        ...
                        "structure_1h": (
                            legacy_bias.structure_1h if legacy_bias else None
                        ),
                        "structure_15m": (
                            legacy_bias.structure_15m if legacy_bias else None
                        ),
                    }
                    if legacy_bias
                    else None
    ...
    print(
        f"Mismatches: {len(mismatches)} ({len(mismatches)/total_bias_updates*100:.1f}%)"
    )
    print()
    ...
    report = {
        "summary": {
            ...
            "match_rate": (
                matches / total_bias_updates if total_bias_updates > 0 else 0.0
            ),
        },
