...
async def replay_historical_data(
    ...
    if merged.empty:
        logger.error("No aligned candles found. Check that GC and DXY have overlapping timestamps.")
        await redis_client.aclose()
        return {
            "candles_published": 0,
            "gc_published": 0,
    ...
    if speed_multiplier == 0:
        ...
        logger.info(f"Backpressure monitoring: pauses if any consumer has >{MAX_PENDING_MESSAGES} unacked messages")
    else:
    ...
    try:
        for idx, row in merged.iterrows():
            ...
            if speed_multiplier == 0 and published_count % BACKPRESSURE_CHECK_INTERVAL == 0:
                while True:
                    has_backpressure, stream_name, length = await check_backpressure(redis_client)
    ...
    if speed_multiplier > 0:
        ...
        logger.info(f"Published {published_count} minutes of data in {duration:.2f} seconds")
        logger.info(f"Speedup: {real_time_duration / duration:.0f}x faster than real-time")
    ...
    async def wait_for_consumer(stream: str, group: str, service_name: str) -> bool:
        ...
        while True:
            ...
            if elapsed > max_wait_seconds:
                logger.warning(f"Timeout waiting for {service_name} after {max_wait_seconds}s")
                return False
            ...
            try:
                ...
                if elapsed < 3 and groups:
                    logger.debug(f"xinfo_groups format for {stream}: {type(groups[0])}, keys: {list(groups[0].keys()) if hasattr(groups[0], 'keys') else 'N/A'}")
                ...
                for group_info in groups:
                    ...
                    if isinstance(group_info, dict):
                        ...
                        lid_key = b"last-delivered-id" if b"last-delivered-id" in group_info else "last-delivered-id"
                        name = group_info.get(name_key, b"")
                        if isinstance(name, bytes):
                            name = name.decode()
                        if name == group:
                            lid = group_info.get(lid_key, b"0")
                            last_delivered_id = lid.decode() if isinstance(lid, bytes) else str(lid)
                            break
                
                if last_delivered_id is None:
                    # Consumer group doesn't exist yet or not found
                    if elapsed - last_log_time >= 10:
                        logger.info(f"Waiting for {service_name}: consumer group '{group}' not found ({elapsed:.0f}s elapsed)")
                ...
                if not stream_last:
                    logger.info(f"{service_name}: stream {stream} is empty, considering caught up")
                    return True
                ...
                if last_delivered_id >= stream_last_id:
                    logger.info(f"{service_name} caught up on {stream} after {elapsed:.1f}s")
                    return True
                
                if elapsed - last_log_time >= 10:  # Log every 10 seconds
                    logger.info(f"Waiting for {service_name}: {last_delivered_id} / {stream_last_id} ({elapsed:.0f}s elapsed)")
                    last_log_time = elapsed
                    
            except Exception as e:
                if elapsed - last_log_time >= 10:
                    logger.warning(f"Error checking {service_name} progress on {stream}: {e}")
                    last_log_time = elapsed
