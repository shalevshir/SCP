...
async def export_signal_history(
    ...
    logger.info(
        f"Querying signal history from {start.isoformat()} " f"to {end.isoformat()}"
    )
    ...
    if format == "json":
        ...
        for signal in signals:
            flat = {
                ...
                "structure_label": signal["features_snapshot"].get("structure_label"),
                ...
                # Diagnostics
                "rejection_passed": signal["diagnostics"]
                .get("rejection_analysis", {})
                .get("passed"),
                "primary_rejection_reason": signal["diagnostics"]
                .get("rejection_analysis", {})
                .get("primary_rejection_reason"),
                "score_gap": signal["diagnostics"]
                .get("rejection_analysis", {})
                .get("score_gap"),
                # IDs
...
async def analyze_rejections(
    ...
    approval_rate_pct = approved_count / total_signals * 100 if total_signals > 0 else 0
    ...
    approval_pct = approved_count / total_signals * 100 if total_signals > 0 else 0
    rejection_pct = rejected_count / total_signals * 100 if total_signals > 0 else 0
...
async def main():
    ...
    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze rejection patterns")
