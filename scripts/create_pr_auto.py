...
def get_commit_messages(limit: int = 10) -> list[str]:
    ...
    exit_code, stdout, _ = run_command(
        ["git", "log", "main..HEAD", "--format=%s%n%b", f"-{limit}"], check=False
    )
    ...
    for commit in commits:
        if "closes #" in commit.lower() or "fixes #" in commit.lower():
            ...
            issue_matches = re.findall(
                r"(?:closes|fixes) #(\d+)", commit, re.IGNORECASE
            )
    ...
    if lint_results:
        ...
        if lint_results["ruff_errors"] == 0 and lint_results["black_errors"] == 0:
            ...
            if lint_results["ruff_warnings"] > 0:
                body_parts.append(
                    f"- ⚠️ {lint_results['ruff_warnings']} warning(s) found (non-blocking)"
                )
        else:
            body_parts.append(
                f"- ⚠️ Linting: {lint_results['ruff_errors']} error(s), {lint_results['black_errors']} formatting issue(s)"
            )
    else:
...
def main() -> int:
    ...
    if success:
        ...
        if test_results.get("coverage"):
            print(f"   📊 Coverage: {test_results['coverage']}")
        print(
            f"   🔍 Linting: {lint_results['ruff_errors']} errors, {lint_results['ruff_warnings']} warnings"
        )
        print(
            f"   ✨ Formatting: {'✅ OK' if lint_results['black_errors'] == 0 else '⚠️ Issues'}"
        )
        print("\n📝 View PR: gh pr view")
        print("🔍 Check status: gh pr checks")
        return 0
    else:
        print("\n❌ Failed to create PR:")
        print(output)
        ...
        if (
            ...
            or "already has a pull request" in output.lower()
        ):
