...
from scripts.replay_databento_historical import DatabentoHistoricalReplay, replay_candles
...
async def main():
    ...
    subprocess.run(
        [
            "docker", "exec", "scp-postgres", "psql", "-U", "scp", "-d", "scp",
            "-c", "TRUNCATE TABLE trades CASCADE",
        ],
        check=False,
    )
