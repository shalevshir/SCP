...
def analyze_structure_labels(
    data_dir: Path, start_date: str, end_date: str
) -> Dict[str, Any]:
    """
    ...
    for file in gc_files:
        ...
        for date, group in df.groupby("date"):
            total = len(group)
            na_count = (group["structure_label"] == "NA").sum() + group[
                "structure_label"
            ].isna().sum()
    ...
    for _, row in worst_days.iterrows():
        print(
            f"   {row['date']}: {row['na_percent']:.1f}% NA ({row['na_count']}/{row['total_candles']} candles)"
        )
...
def analyze_htf_bias_distribution(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    neutral_percent = (
        (bias_counter["neutral"] / total_signals * 100) if total_signals > 0 else 0
    )
...
def analyze_scoring_factors(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if total_signals > 0:
        ...
        print(
            f"   rejection_candle: {rejection_candle_zeros}/{total_signals} zeros ({rej_zero_percent:.1f}%)"
        )
        print(
            f"   volume_spike: {volume_spike_zeros}/{total_signals} zeros ({vol_zero_percent:.1f}%)"
        )
...
def analyze_seasonality_gating(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    blocked_percent = (
        (blocked_count / len(seasonality_stats) * 100) if seasonality_stats else 0
    )
    ...
    print(
        f"   Blocked by seasonality: {blocked_count}/{len(seasonality_stats)} ({blocked_percent:.1f}%)"
    )
    ...
    print(
        f"   0.5 - 0.7: {((df_stats['seasonality_factor'] >= 0.5) & (df_stats['seasonality_factor'] < 0.7)).sum()}"
    )
    print(
        f"   0.7 - 0.9: {((df_stats['seasonality_factor'] >= 0.7) & (df_stats['seasonality_factor'] < 0.9)).sum()}"
    )
    ...
    if not low_seasonality.empty:
        ...
        for _, row in low_seasonality.head(5).iterrows():
            print(
                f"      {row['timestamp']} ({row['setup_type']}): {row['seasonality_factor']:.2f}"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python diagnose_backtest_results.py <results_json_path> [data_dir] [start_date] [end_date]"
        )
        sys.exit(1)
    ...
    print(
        f"   Total PnL: ${results.get('metadata', {}).get('total_pnl_dollars', 0):.2f}"
    )
    ...
    if data_dir.exists():
        results_dict["structure_analysis"] = analyze_structure_labels(
    ...
    if struct_analysis.get("avg_na_percent", 0) > 30:
        critical_issues.append(
            f"Structure NA% too high: {struct_analysis['avg_na_percent']:.1f}%"
        )

    htf_analysis = results_dict.get("htf_bias_analysis", {})
    if htf_analysis.get("neutral_percent", 0) > 60:
        critical_issues.append(
            f"HTF neutral bias too high: {htf_analysis['neutral_percent']:.1f}%"
        )
    ...
    if season_analysis.get("blocked_percent", 0) > 20:
        critical_issues.append(
            f"Seasonality blocking too many signals: {season_analysis['blocked_percent']:.1f}%"
        )
