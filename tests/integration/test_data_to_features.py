...
async def test_features_include_correlation(
    ...
    features_with_corr = [
        f for f in features_list[-10:] if f.dxy_correlation is not None
    ]

    assert (
        len(features_with_corr) > 0
    ), "DXY correlation should be computed after warmup"

    # Verify correlation is negative (inverse relationship)
    latest_corr = features_with_corr[-1].dxy_correlation
    assert (
        latest_corr is not None
    ), "Latest correlation should not be None (filtered above)"
