...
class TestMinRiskConfigPassthrough:
    ...
    def test_replay_loop_passes_config_parameter(self):
        ...
        """
        replay_loop_path = Path(__file__).parent.parent.parent.parent / "backtester" / "replay_loop.py"
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name) and node.func.id == "create_trade_from_entry":
        ...
        missing_config = [
            call for call in calls_found 
            if "config" not in call["kwargs"]
        ]
    ...
    def test_pipeline_passes_config_parameter(self):
        ...
        """
        pipeline_path = Path(__file__).parent.parent.parent.parent / "backtester" / "pipeline.py"
