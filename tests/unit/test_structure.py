...
from feature_engine.state import StructureState
...
class TestStructureLabels:
    ...
    def test_bearish_structure(self) -> None:
        ...
        bearish_labels = labels.iloc[4:10][labels.iloc[4:10].isin(["LH", "LL"])]
    ...
    def test_incremental_matches_vectorized(self) -> None:
        ...
        for i in range(len(df)):
            ...
            assert vec_label == inc_label, (
                f"Mismatch at index {i}: vectorized={vec_label}, incremental={inc_label}"
            )
        
        # Verify that both produce at least some labels
        assert vectorized_labels.notna().sum() > 0, "Vectorized should produce some labels"
        assert sum(1 for l in incremental_labels if l is not None) > 0, (
