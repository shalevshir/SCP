...
class TestBacktestProcessor:
    ...
    def test_no_look_ahead_bias(self, sample_data):
        ...
        features_without_spike = [
            f for f, _ in processor.iterate_with_context(gc_df, dxy_df)
        ]
        features_with_spike = [
            f for f, _ in processor.iterate_with_context(gc_df_with_spike, dxy_df)
        ]
        ...
        for i in range(len(features_without_spike) - 1):
            ...
            assert (
                abs(features_without_spike[i]["vwap"] - features_with_spike[i]["vwap"])
                < 0.01
            )
    ...
    def test_session_boundary_vwap_reset(self):
        ...
        before_reset_base = datetime(2025, 1, 15, 13, 0, tzinfo=UTC)
        before_times = [before_reset_base + timedelta(minutes=i) for i in range(20)]
        # At/after 08:20 ET (13:20 UTC = 08:20 ET)
        after_reset_base = datetime(2025, 1, 15, 13, 20, tzinfo=UTC)
        ...
        processor = BacktestProcessor(
            timeframe="1m", session_reset=True, warmup_period=5
        )
        # Unpack tuples: extract just features
        features_list = [f for f, _ in processor.iterate_with_context(gc_df, dxy_df)]
    ...
    def test_structure_labels_delayed_to_prevent_lookahead_bias(self, sample_data):
        ...
        if len(features_list) >= swing_window:
            ...
            for idx in range(len(features_list) - swing_window, len(features_list)):
                ...
                assert (
                    pd.isna(feature["structure_label"])
                    or feature["structure_label"] is None
                ), (
                ...
                assert (
                    pd.isna(feature["structure_type"])
                    or feature["structure_type"] is None
                ), (
