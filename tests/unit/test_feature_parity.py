...
class TestFeatureParity:
    ...
    def test_vwap_parity(self, sample_candles):
        ...
        for inc, vec in zip(inc_vwaps, vec_vwaps, strict=False):
    ...
    def test_ema_parity(self, sample_candles):
        ...
        for i in range(len(inc_ema9)):
            assert (
                abs(inc_ema9[i] - vec_ema9[i]) < 0.0001
            ), f"EMA9 mismatch at {i}: {inc_ema9[i]} vs {vec_ema9[i]}"
            assert (
                abs(inc_ema20[i] - vec_ema20[i]) < 0.0001
            ), f"EMA20 mismatch at {i}: {inc_ema20[i]} vs {vec_ema20[i]}"
            assert (
                abs(inc_ema50[i] - vec_ema50[i]) < 0.0001
    ...
    def test_backtest_processor_vs_incremental_vwap(self, sample_candles):
        ...
        dxy_df = pd.DataFrame(
            ...
            index=pd.DatetimeIndex(
                [c.timestamp for c in dxy_candles], name="timestamp"
            ),
        )
    ...
    def test_backtest_processor_vs_incremental_ema(self, sample_candles):
        ...
        for i, (bt_feat, inc_feat) in enumerate(zip(backtest_features, inc_features, strict=False)):
            for period in [9, 20, 50]:
                ...
                assert (
                    abs(bt_feat[ema_key] - inc_feat[ema_key]) < 0.01
...
class TestEdgeCases:
    ...
    def test_session_boundary(self):
        ...
        before_reset_time = datetime(2025, 1, 15, 13, 0, tzinfo=UTC)
        gc1 = Candle(
            before_reset_time,
            2000.0,
            2002.0,
        ...
        reset_time = datetime(2025, 1, 15, 13, 20, tzinfo=UTC)
        gc2 = Candle(
