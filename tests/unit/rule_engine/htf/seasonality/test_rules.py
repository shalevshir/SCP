...
class TestGetSeasonalityPeriod:
    ...
    def test_october_returns_october_period(self) -> None:
        ...
        timestamp = datetime(2024, 10, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
    ...
    def test_november_returns_november_december_period(self) -> None:
        ...
        timestamp = datetime(2024, 11, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
    ...
    def test_december_returns_november_december_period(self) -> None:
        ...
        timestamp = datetime(2024, 12, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
        assert result == "november_december"
    ...
    def test_february_returns_other_period(self) -> None:
        ...
        timestamp = datetime(2024, 2, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
        assert result == "other"
    ...
    def test_april_returns_other_period(self) -> None:
        ...
        timestamp = datetime(2024, 4, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
        assert result == "other"

    def test_may_returns_other_period(self) -> None:
        """May (month 5) should return 'other' period."""
        timestamp = datetime(2024, 5, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
        assert result == "other"
    ...
    def test_year_transition_december_to_january(self) -> None:
        ...
        dec_timestamp = datetime(2024, 12, 31, 23, 59, tzinfo=timezone.utc)
        jan_timestamp = datetime(2025, 1, 1, 0, 1, tzinfo=timezone.utc)
    ...
    def test_leap_year_february(self) -> None:
        ...
        timestamp = datetime(2024, 2, 29, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
        assert result == "other"
    ...
    def test_different_timezones_same_month(self) -> None:
        ...
        utc_timestamp = datetime(2024, 9, 15, 12, 0, tzinfo=timezone.utc)
    ...
    def test_works_with_future_dates(self) -> None:
        ...
        future_timestamp = datetime(2030, 11, 15, 12, 0, tzinfo=timezone.utc)
    ...
    def test_works_with_past_dates(self) -> None:
        ...
        past_timestamp = datetime(2020, 9, 15, 12, 0, tzinfo=timezone.utc)
...
class TestGetSeasonalityConfig:
    ...
    def test_november_december_config_returns_relaxed_thresholds(self) -> None:
        ...
        assert config["max_losses"] == 2
        assert "trend" in config["description"].lower() or "november" in config["description"].lower()
    ...
    def test_other_config_returns_standard_thresholds(self) -> None:
        ...
        assert config["dxy_corr_threshold"] == -0.6
        assert config["max_losses"] == 2
        assert "standard" in config["description"].lower() or "other" in config["description"].lower()

    def test_all_configs_have_required_keys(self) -> None:
        """All configs should contain required keys."""
        required_keys = {"min_score_threshold", "dxy_corr_threshold", "max_losses", "description"}
...
class TestSeasonalityIntegration:
    ...
    @pytest.mark.parametrize("month,expected_period", [
        (1, "other"),
        (2, "other"),
        (3, "other"),
        (4, "other"),
    ...
    def test_all_months_parameterized(self, month: int, expected_period: SeasonalityPeriod) -> None:
        """Parameterized test covering all 12 months."""
        timestamp = datetime(2024, month, 15, 12, 0, tzinfo=timezone.utc)
        result = get_seasonality_period(timestamp)
