...
class TestBoundaryDetection:
    ...
    def test_15m_boundary_not_at_other_minutes(self, calculator):
        ...
        for minute in [0, 5, 10, 13, 15, 20, 28, 30, 43, 45, 58]:
            ...
            assert calculator._is_15m_boundary(ts) is False, f"Minute {minute} should not be 15M boundary"
    ...
    def test_1h_boundary_not_at_other_minutes(self, calculator):
        ...
        for minute in [0, 14, 29, 30, 44, 45, 58]:
            ts = datetime(2025, 1, 1, 10, minute, tzinfo=UTC)
            assert calculator._is_1h_boundary(ts) is False, f"Minute {minute} should not be 1H boundary"
...
class TestBufferManagement:
    ...
    def test_15m_buffer_limits_size_to_200_bars(self, calculator):
        ...
        expected_first = datetime(2025, 1, 1, 10, 0, tzinfo=UTC) + timedelta(minutes=50 * 15)
        assert first_timestamp == expected_first
...
class TestHTFBiasComputation:
    ...
    def test_update_computes_bias_at_15m_boundary_when_features_available(
        ...
        mock_bias = HTFBias(bias="bullish", direction="long", score=8.0, confidence="high")
    ...
    @patch('rule_engine.htf.streaming.compute_htf_bias')
    def test_update_handles_computation_error_gracefully(self, mock_compute_bias, calculator):
...
class TestStateConsistency:
    ...
    @patch('rule_engine.htf.streaming.compute_htf_bias')
    def test_features_stay_synchronized_across_updates(self, mock_compute_bias, calculator):
    ...
    @patch('rule_engine.htf.streaming.compute_htf_bias')
    def test_current_htf_bias_persists_between_updates(self, mock_compute_bias, calculator):
    ...
    def test_get_current_bias_returns_most_recent_bias(self, calculator):
        ...
        # Set a bias
        test_bias = HTFBias(bias="bullish", direction="long", score=8.0, confidence="high")
