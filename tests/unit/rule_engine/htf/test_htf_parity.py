...
class TestHTFBiasParity:
    ...
    def timestamp_september(self) -> pd.Timestamp:
        ...
        return pd.Timestamp(datetime(2025, 9, 15, 10, 0, tzinfo=timezone.utc))
    ...
    def test_full_htf_parity_integration(
        ...
        for key in dict1:
            if isinstance(dict1[key], float) and dict1[key] is not None:
                assert dict1[key] == pytest.approx(dict2[key], abs=0.01), f"Field {key} mismatch"
            else:
                assert dict1[key] == dict2[key], f"Field {key} mismatch"
